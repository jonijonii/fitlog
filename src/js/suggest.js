...
(function (FitLog) {
  ...
  /** '으로/로' 만 규칙이 다르다 — 받침이 없거나 ㄹ 이면 '로' 다 (시금치로, 굴로, 김으로). */
  function withRo(word) {
    var w = plain(word);
    if (!hasJong(w)) return w + '로';
    var code = w.charCodeAt(w.length - 1);
    if (code >= 0xAC00 && code <= 0xD7A3 && (code - 0xAC00) % 28 === 8) return w + '로';
    return w + '으로';
  }

  /** 영양소 이름 잇기. 둘이면 '칼륨과 칼슘', 셋 이상이면 쉼표로 잇는다. */
  function joinNutrients(names) {
    if (!names.length) return '';
    if (names.length === 1) return plain(names[0]);
    if (names.length === 2) {
      return withParticle(names[0], '과', '와') + ' ' + plain(names[1]);
    }
  ...
  function foodsFor(state, gap, opts) {
    var o = opts || {};
    var limit = o.limit === undefined ? PER_NUTRIENT : o.limit;
  ...
  function build(state, endDateKey, dayCount) {
    ...
    if (report.loggedDays < MIN_DAYS) {
      return {
        ...
        gaps: [], overs: [], lines: [], notes: [], uncovered: [], headline: ''
      };
    }
    ...
    /* 영양소별로 '뭘 먹으면 채워지나' 를 한 줄씩 만든다.
     *
     * 음식별 카드로 묶었다가 되돌린 자리다. 사용자가 원한 건 기록 버튼이 아니라
     * **다음 주 식단을 고를 때 참고할 정보** 였다:
     *   "이번 주엔 칼륨과 칼슘이 부족하네.
     *    칼륨은 감자, 고구마, 시금치로 보충할 수 있어."
     * 그래서 영양소를 앞에 세우고 음식 이름만 댄다. 버튼도 두지 않는다. */
    var lines = [];
    var uncovered = [];

    gapList.forEach(function (gap) {
      var picks = foodsFor(state, gap);

      // 채울 음식이 없으면 부정확한 제안을 하느니 사실만 알린다
      if (!picks.length) {
        uncovered.push(uncoveredNote(state, gap));
        return;
      }

      var names = picks.map(function (card) { return plain(card.name); });
      lines.push({
        key: gap.key,
        name: gap.name,
        foods: picks,
        message: withParticle(gap.name, '은', '는') + ' ' +
          withRo(names.join(', ')) + ' 보충할 수 있어.'
      });
    });

    var headline = '이번 주엔 ' + withParticle(
      joinNutrients(gapList.map(function (g) { return g.name; })), '이', '가') + ' 부족하네.';
